...
interface JourneysFactory {
	getJourneys(): Journey[],
	getAmountOfCountries(): number
}

app.factory('JourneysFactory', function (): JourneysFactory {
	...
	journeys.forEach(journey => {
		let end: Date = journey.end || new Date();

		journey.days = Math.round(Math.abs(end.getTime() - journey.start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
	});
	
	return {
		getJourneys: function (): Journey[] {
			return journeys;
		},
		getAmountOfCountries: function() : number {
			...
			countryNames = countryNames.filter((countryName, index) => {
